...
def custom_loss(y_true, y_pred, depth=1, verbose=False):
    ...
    if verbose:
        ...
        print('Rhyme counts (last to first): ', loss_c)

    # loss = tf.add(categorical_crossentropy(y_true, y_pred), rhyme_counts * tf.float32(0.1))
    return loss_c
...
class CustomModel:
    ...
    def custom_loss_inner(self, y_true, y_pred):
        ...
        y_true2 = table.lookup(y_true2)
        y_pred2 = table.lookup(y_pred2)

        loss_custom = custom_loss(y_true2, y_pred2)
...
if __name__ == "__main__":
    ...
    # lstm.train(x_train, y_train, epochs=1, batch_size=512, verbose=1)
    lstm.load()
